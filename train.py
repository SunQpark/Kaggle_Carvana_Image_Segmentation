def train(self, epoch, redundancy, keep_prob):  # 200 per training(0~199), 8 per validation(200-207)
    ...
    with tf.name_scope("loss"):
        loss = 1 - self.dice_coe(output=hypothesis, target=self.y, axis=[0, 1, 2, 3])
        # loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=hypothesis, labels=self.y), name="loss")
